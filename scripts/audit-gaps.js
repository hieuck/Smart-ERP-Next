...
function countTableRows(markdown) {
  return markdown
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter((line) => line.startsWith("|") && !line.includes("---"))
    .filter((line) => !/^\|\s*(Gap|Item|Role area)\s*\|/.test(line)).length;
}
...
function auditGaps(repoRoot = REPO_ROOT) {
  ...
  const completedRows = countTableRows(completedSection);
  if (counts.completed !== completedRows) {
    findings.push(
      `Completed summary says ${counts.completed}, but Completed table has ${completedRows} rows.`,
    );
  }
  ...
  const remainingRows = countTableRows(remainingSection);
  if (counts.remaining !== remainingRows) {
    findings.push(
      `Remaining summary says ${counts.remaining}, but Remaining table has ${remainingRows} rows.`,
    );
  }
...
module.exports = { auditGaps, countTableRows, extractSummaryCounts };
